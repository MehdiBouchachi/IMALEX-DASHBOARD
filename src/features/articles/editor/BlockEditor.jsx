...
/* Small yellow badge for section flags */
const FlagBadge = styled.span`
  ...
  font-size: 11px;
  font-weight: 700;
  color: color-mix(in oklab, var(--color-yellow-700) 85%, var(--color-grey-0));
  background: color-mix(in oklab, var(--color-yellow-100) 80%, transparent);
  border: 1px solid
    color-mix(in oklab, var(--color-yellow-700) 35%, var(--color-yellow-100));
  box-shadow: inset 0 0 0 1px
    color-mix(in oklab, var(--color-yellow-700) 10%, transparent);
  &::before {
    content: "⚑";
  }
`;
...
const BlockRow = styled.div`
  ...
  box-shadow: ${(p) =>
    ...
    /* yellow rail when flagged */
      ${(p) =>
        p.$flagged
          ? "inset 4px 0 0 0 var(--color-yellow-700), inset 5px 0 0 0 color-mix(in oklab, var(--color-yellow-100) 70%, transparent)"
          : "none"};
...
const GroupPill = styled.span`
  ...
  padding: 2px 9px;
  border-radius: 999px;
  border: 1px solid color-mix(in oklab, var(--g) 55%, var(--color-grey-100));
  background: linear-gradient(
      180deg,
...
/* Small flag comment container */
const FlagWrap = styled.div`
  margin: 8px 0 0;
  border: 1px dashed
    color-mix(in oklab, var(--color-yellow-700) 30%, var(--color-yellow-100));
  background: color-mix(in oklab, var(--color-yellow-100) 70%, transparent);
  border-radius: 10px;
  padding: 8px;
  display: grid;
  gap: 8px;
...
const shapeFor = (t) => {
  const base = { id: uid(), flagged: false, flagComment: "" };
  if (t === "ul" || t === "ol") return { ...base, t, x: [""] };
  if (t === "img") return { ...base, t, src: "", alt: "", cap: "" };
  if (t === "blockquote")
    return {
      ...base,
      t,
  ...
  if (t === "callout") return { ...base, t, v: "tip", x: "" };
  if (t === "code") return { ...base, t, x: "" };
  if (t === "h2" || t === "h3") return { ...base, t, x: "" };
  return { ...base, t: "p", x: "" };
};
...
export default function BlockEditor({ value = [], onChange }) {
  const blocks = useMemo(
    () =>
      value.map((b) =>
        b.id ? b : { ...shapeFor(b.t || "p"), ...b, id: uid() }
      ),
    [value]
  );
  ...
  /* count flags in a section (h2 itself + its children) */
  const flagsInSection = (sec) => {
    let n = 0;
    if (sec.h2?.flagged) n += 1;
    sec.items.forEach(({ block }) => {
      if (block.flagged) n += 1;
    });
    return n;
  };
  ...
  return (
    <Wrap>
      ...
      {sections.map((s, k) => {
        ...
        const flagCount = flagsInSection(s);
          ...
          <SectionCard key={key}>
            <SectionHeader>
              ...
              <SectionMeta>
                ...
                {/* show yellow flag count when collapsed (as requested) */}
                {!isOpen && flagCount > 0 ? (
                  <FlagBadge title="Requested changes">{flagCount}</FlagBadge>
                ) : null}
              </SectionMeta>
            </SectionHeader>
...
function EditableRow({
  ...
  const toggleFlag = () => {
    if (block.flagged) {
      update(idx, { flagged: false, flagComment: "" });
    } else {
      update(idx, { flagged: true });
    }
  };
    ...
    >
      <RowInner onKeyDown={(e) => onKeyMove(idx, e)}>
        ...
        <div>
            ...
            {block.flagged && (
              <span
                ...
                style={{
                  ...
                  fontWeight: 700,
                  color:
                    "color-mix(in oklab, var(--color-yellow-700) 85%, var(--color-grey-0))",
                  background:
                    "color-mix(in oklab, var(--color-yellow-100) 80%, transparent)",
                  border:
                    "1px solid color-mix(in oklab, var(--color-yellow-700) 35%, var(--color-yellow-100))",
                }}
              >
                ⚑ change requested
              </span>
          ...
          {block.flagged && (
            <FlagWrap>
              <Textarea
                rows={2}
                placeholder="Add a short reviewer note…"
                value={block.flagComment || ""}
                onChange={(e) => update(idx, { flagComment: e.target.value })}
              />
              <div
        ...
        <Toolbar className="blk-toolbar">
          <Button
            size="small"
            variation={block.flagged ? "secondary" : "primary"}
            onClick={toggleFlag}
            title={block.flagged ? "Resolve / remove flag" : "Request changes"}
          >
            {block.flagged ? "✓ Resolve" : "⚑ Request changes"}
          </Button>
